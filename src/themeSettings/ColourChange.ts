...
export default class ColourChange {
  ...
  public static colourRides = (
     ...
     */
    const filterRidesToTheme = (
      initialRidesToTheme = ridesToPaint ||
        rideController.selectedRides.get() ||
        []
    ) => {
      let finalRidesToTheme: Ride[];
      if (settingsController.getValue("repaintExistingRides") === false) {
        finalRidesToTheme = initialRidesToTheme?.filter(
          (ride) => rideController.paintedRides.get()?.indexOf(ride) === -1
        );
    ...
    Object.keys(groupedRides).forEach((group, i) => {
      ...
      const colours = currentMode.applyTheme(currentTheme, {
        ...
        index: i,
        naturalSupports: settingsController.naturalSupports.get(),
      });
      if (!colours) return;
  ...
  public static markAllRidesAsPainted = (rc: RideController) => {
    rc.all.get().forEach((r) => this.markRideAsHavingBeenPainted(r, rc));
  };
   ...
   */
  private static markRideAsHavingBeenPainted = (
    ride: Ride,
    rc: RideController
  ) => {
  ...
  public static changeRideStationStyle = (
    rides: Ride[],
