 ...
 */
  private static setRideColourPart = (ride: Ride, part:number, colour: number) => {
    ...
    if (colour >= 0 && colour < 32) {
      ...
      console.log(`Colour not changed for ride ${ride.name} for part ${part}. Given colour ${colour} is outside the acceptable range of 0-31. To keep a colour value unchanged without getting this warning, pass in '-1' for the colour value.`);
    }
  };
...
public static setRideColour = (
  ...
  public static setRandomTrackColour = (ride:Ride): void => {
    ColourChange.setRideColourTracks(ride, context.getRandom(0, 31), context.getRandom(0, 31), context.getRandom(0, 31));
  };
